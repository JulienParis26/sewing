...
class FabricResourceIT {
    ...
    void createFabric() throws Exception {
        ...
        restFabricMockMvc
            .perform(
                post(ENTITY_API_URL).with(csrf()).contentType(MediaType.APPLICATION_JSON).content(TestUtil.convertObjectToJsonBytes(fabric))
            )
    ...
    void putWithMissingIdPathParamFabric() throws Exception {
        ...
        restFabricMockMvc
            .perform(
                put(ENTITY_API_URL).with(csrf()).contentType(MediaType.APPLICATION_JSON).content(TestUtil.convertObjectToJsonBytes(fabric))
            )
    ...
    void deleteFabric() throws Exception {
        ...
        // Delete the fabric
        restFabricMockMvc
            .perform(delete(ENTITY_API_URL_ID, fabric.getId()).with(csrf()).accept(MediaType.APPLICATION_JSON))
